...
resource "libvirt_domain" "workers" {
  ...
  disk {
    block_device = "/dev/${element(libvirt_volume.worker_root_disk.*.pool, count.index)}/${element(libvirt_volume.worker_root_disk.*.name, count.index)}"
  }

  disk {
    block_device = "/dev/${element(libvirt_volume.worker_ceph_metadata_disk.*.pool, count.index)}/${element(libvirt_volume.worker_ceph_metadata_disk.*.name, count.index)}"
  }
  ...
  disk {
    file = "/var/lib/libvirt/images/worker.iso"
  }
